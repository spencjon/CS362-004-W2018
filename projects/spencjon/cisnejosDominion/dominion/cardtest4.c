...
int checkVillageCard(int currentPlayer, struct gameState* gameReturned){    
    int returned;
    ...
    if(assertStandardDom(gameExpected.handCount[currentPlayer] == gameReturned->handCount[currentPlayer], "num cards inequal")){
        return 1;
    }
    return 0;
}
...
int main () {
  ...
  printf ("SIMPLE FIXED TESTS: Village Card\n");
  for (p = 2; p < 5; p++)
  {
    for(i = 0; i < p; i++)
    {
        ...
        G.hand[i][0] = 14; //place village card into hand
        G.whoseTurn = i; //it's that person's turn
        
        if(checkVillageCard(i, &G))
        {
